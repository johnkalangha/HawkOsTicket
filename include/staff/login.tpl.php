...
?>
  ...
  <div class="login-box-body">
    <p class="login-box-msg"><h3><?php echo Format::htmlchars($msg); ?></h3>
    <div class="banner"><small><?php echo ($content) ? Format::display($content->getLocalBody()) : ''; ?></small></div></p>
    <form action="login.php" method="post" id="login">
        <?php csrf_token(); ?>
        <input type="hidden" name="do" value="scplogin">
        <div class="form-group has-feedback">
            <input class="form-control" type="text" name="userid" id="name" value="<?php
            echo $info['userid']; ?>" placeholder="<?php echo __('Email or Username'); ?>"
            autofocus autocorrect="off" autocapitalize="off">
            <span class="glyphicon glyphicon-envelope form-control-feedback"></span>
        </div>
        <div class="form-group has-feedback">
            <input class="form-control" type="password" name="passwd" id="pass" placeholder="<?php echo __('Password'); ?>" autocorrect="off" autocapitalize="off">
            <span class="glyphicon glyphicon-lock form-control-feedback"></span>
        </div>
        <div class="row">
            <button class="btn btn-primary btn-flat pull-right" type="submit" name="submit"><i class="icon-signin"></i>
            <?php echo __('Log In'); ?>
            </button>
        </div>
    </form>
    ...
    <?php if ($show_reset && $cfg->allowPasswordReset()) { ?>
        <a href="pwreset.php"><?php echo __('Forgot My Password'); ?></a><br/>
    <?php } ?>
